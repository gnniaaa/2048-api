...
X_train, X_test, Y_train, Y_test = train_test_split(image, label, test_size=0.01,shuffle=False)
    ...
    for i, (images, labels) in enumerate(trainloader):
        ...
        if i % 1000 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    epoch, i * batch_size, len(trainloader.dataset),
                           100. * i / len(trainloader), loss.item()))
    print("train accuracy: ", correct/float(X_train.shape[0]))
torch.save(model.state_dict(),'model256.pth' )
